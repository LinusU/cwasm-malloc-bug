...
# Relase build
# RUN clang --sysroot=/sysroot --target=wasm32-unknown-unknown-wasm -Oz     -o test.wasm -nostartfiles -fvisibility=hidden -Wl,--no-entry,--demangle,--allow-undefined,--export=malloc,--export=free,--strip-all -- /src/compiler-rt/lib/builtins/*.c

# Debug build
RUN clang --sysroot=/sysroot --target=wasm32-unknown-unknown-wasm -O0 -g3 -o test.wasm -nostartfiles -fvisibility=hidden -Wl,--no-entry,--demangle,--allow-undefined,--export=malloc,--export=free             -- /src/compiler-rt/lib/builtins/*.c

CMD base64 --wrap=0 test.wasm
